...
public class Application {
     ...
     * -> 추상 메소드에 어노테이션으로 쿼리를 입력하고, getMapper로 쿼리를 불러와 실해
     * -> SQL 구문을 메소드처럼 호출해서 사용하는 장점
     * -> 간결해서 가독성이 좋다.
     * -> 쿼리문 작성이 힘들다.
     *
     * 2. xml 파일을 사용하는 방법
     * -> xml애 직접 SQL문을 정의하여 가져와서 사용
    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        Environment environment = new Environment(
                ...
                new PooledDataSource(DRIVER, URL, USER, PASSWORD) // ConnectionPool 사용 유무
        );

        // 2. configuration 객체에 환경정보 담기
        Configuration configuration = new Configuration(environment);

        // 3. Configuration에 Mapper 등록
        configuration.addMapper(Mapper.class);

        /*
        * SqlSessionFactory : SqlSession 객체를 생성하기 위한 팩토리 역할의 인터페이스
        *
        * SqlSessionFactoryBuilder : SqlSessionFactory 인터페이스 타이의 하위 구현 객체를 생성하기 위한 빌드 역할
        *
        ...
        *           Auto commit에 대한 옵션을 false로 지정 (권장되는 방식)
